...
class ArmRightWithHandController:
    ...
    def __init__(self, model: mujoco.MjModel, data: mujoco.MjData, config: ArmConfig):
        ...
        # self.hand_grasp_angles = np.array([1.5, 0.3, 1.0, 0.6, 0.8, 1.1, 0.8, 1.1, 0.8, 1.1, 0.8, 1.1])
        # self.hand_grasp_angles = np.array([1.5, 0.3, 1.0, 0.6, 0.8, 1.1, 0.8, 1.1, 1.1, 1.4, 1.1, 1.4])
        # self.hand_grasp_angles = np.array([
        #     0.8, 0.9, 1.2, 1.1,   # 拇指（yaw, pitch, 近节, 远节）
        #     0.8, 1.1,             # 食指
        #     0.8, 1.1,             # 中指
        #     1.1, 1.4,             # 无名指
        #     1.1, 1.4              # 小指
        # ])
        self.hand_grasp_angles = np.array([
            # 拇指 4 个关节: yaw, pitch, inter, distal
            0.3,  1.0,  1.2,  1.0,   # 拇指：减小 yaw，多用 pitch+指节去弯（像一只爪子）
            # 食指（proximal, intermediate）
            0.6,  0.9,
            # 中指
            0.6,  0.9,
            # 无名指
...
class ArmLeftWithHandController:
    ...
    def __init__(self, model: mujoco.MjModel, data: mujoco.MjData, config: ArmConfig):
        ...
        # self.hand_grasp_angles = np.array([1.3, 0.1, 0.8, 0.4, 0.6, 0.9, 0.6, 0.9, 0.6, 0.9, 0.6, 0.9])
        # self.hand_grasp_angles = np.array([1.5, 0.1, 0.8, 0.4, 0.6, 0.9, 0.6, 0.9, 0.6, 0.9, 0.6, 0.9])
        self.hand_grasp_angles = np.array([1.5, 0.1, 1.1, 0.7, 0.9, 1.2, 0.9, 1.2, 0.9, 1.2, 0.9, 1.2])
