...
class ArmConfig:
    ...
    def from_yaml(cls, config_path: str) -> 'ArmConfig':
        ...
        instance.joint_limits = {
            ...
            'left_shoulder_roll':  [-1.5882,  2.2515],  # 注意这项与右臂不同
            ...
            'left_hand_thumb_1': [-1.0472,  0.724312],
            'left_hand_thumb_2': [-1.74533, 0.0],

            # 中指（2 个关节）
            'left_hand_middle_0': [0.0,     1.5708],
            'left_hand_middle_1': [0.0,     1.74533],

            # 食指（2 个关节）
            'left_hand_index_0': [0.0,     1.5708],
            'left_hand_index_1': [0.0,     1.74533], 
        ...
        instance.joint_torque_limits = {
            ...
            # ===== 左手：三指夹爪 =====
            # 拇指 3 节
            'left_hand_thumb_0': 2.45,
            'left_hand_thumb_1': 1.40,
            'left_hand_thumb_2': 1.40,

            # 食指 2 节
            'left_hand_index_0': 1.40,
            'left_hand_index_1': 1.40,

            # 中指 2 节
            'left_hand_middle_0': 1.40,
            'left_hand_middle_1': 1.40,

            # ===== 右手：三指夹爪 =====
            'right_hand_thumb_0': 2.45,
            'right_hand_thumb_1': 1.40,
            'right_hand_thumb_2': 1.40,

            'right_hand_index_0': 1.40,
            'right_hand_index_1': 1.40,

            'right_hand_middle_0': 1.40,
            'right_hand_middle_1': 1.40,
        }

        # 设置手指默认角度（完全展开状态）
        instance.hand_default_angles = np.zeros(7)  # 单手7个手指关节

        # 设置手部PD控制增益
        instance.hand_kps = instance.kps[36:43]  # 手部关节的kp
        instance.hand_kds = instance.kds[36:43]  # 手部关节的kd
        # instance.hand_kps = np.array([80, 80, 80, 60, 60, 60, 60], dtype=np.float32)
        # instance.hand_kds = np.array([ 1,  1,  1, 0.5,0.5,0.5,0.5], dtype=np.float32)
        # instance.hand_kps = np.array([60, 60, 60, 40, 40, 40, 40], dtype=np.float32)
        # instance.hand_kds = np.array([ 2,  2,  2, 1.0,1.0,1.0,1.0], dtype=np.float32)
...
class ArmBaseController:
    ...
    def __init__(self, model: mujoco.MjModel, data: mujoco.MjData, config: ArmConfig, arm_side: str):
        ...
        if arm_side == "left":
            ...
            self.hand_joint_indices = list(range(22, 29))  # 左手关节
        else:  # arm_side == "right"
            self.arm_joint_indices = list(range(29, 36))  # 右臂关节
            self.hand_joint_indices = list(range(36, 43))  # 右手关节
    ...
    def update(self) -> None:
        ...
        scale = min(1.0, max(0.2, error_norm))
        pos_error *= scale
...
class ArmRightWithHandController:
    ...
    def __init__(self, model: mujoco.MjModel, data: mujoco.MjData, config: ArmConfig):
        ...
        # 3 指夹爪：拇指对向食指+中指，三指都弯起来夹住
        # self.hand_grasp_angles = np.array([
        #     # 拇指 thumb_0, thumb_1, thumb_2
        #     0.55,   # right_hand_thumb_0_joint   （绕掌根旋转，让拇指朝向食/中指）
        #     0.40,   # right_hand_thumb_1_joint   （拇指根部屈曲，朝掌心弯）
        #     -1.00,   # right_hand_thumb_2_joint   （末节弯曲：范围 [-1.7453, 0]，取中等弯）

        #     # 食指 index_0, index_1 （两节都中等弯曲）
        #     0.90,   # right_hand_index_0_joint   （近节，范围 [0, 1.5708]）
        #     1.10,   # right_hand_index_1_joint   （远节，范围 [0, 1.7453]）

        #     # 中指 middle_0, middle_1 （和食指差不多，夹爪更平）
        #     0.95,   # right_hand_middle_0_joint
        #     1.15,   # right_hand_middle_1_joint
        # ], dtype=float)

        self.hand_grasp_angles = np.array([
            # 拇指（更紧抓握）
            0.80,     # thumb_0   （range: [-1.0472,  1.0472]）
            0.65,     # thumb_1   （range: [-1.0472,  0.724312]）
            -1.50,    # thumb_2   （range: [-1.74533, 0.0]）

            # 食指
            1.40,     # index_0   （range: [0.0,     1.5708]）
            1.50,     # index_1   （range: [0.0,     1.74533]）

            # 中指
            1.40,     # middle_0  （range: [0.0,     1.5708]）
            1.50,     # middle_1  （range: [0.0,     1.74533]）
        ], dtype=float)

        # 右手关节索引 (索引36-42)
        self.hand_joint_indices = list(range(36, 43))
        # 右手关节名称
...
class ArmLeftWithHandController:
    ...
    def __init__(self, model: mujoco.MjModel, data: mujoco.MjData, config: ArmConfig):
        ...
        # 左手关节索引 (索引22-28)
        self.hand_joint_indices = list(range(22, 29))

        # 3 指夹爪：拇指对向食指+中指，三指都弯起来夹住
        self.hand_grasp_angles = np.array([
            # 拇指（正方向也是朝掌心弯）
            0.55,   # left_hand_thumb_0_joint
            0.40,   # left_hand_thumb_1_joint
            1.00,   # left_hand_thumb_2_joint （左手 thumb_2 range [0, 1.7453]）

            # 食指
            -0.90,   # left_hand_index_0_joint   （range [-1.5708, 0]）
            -1.10,   # left_hand_index_1_joint   （range [-1.7453, 0]）

            # 中指
            -0.95,   # left_hand_middle_0_joint
            -1.15,   # left_hand_middle_1_joint
        ], dtype=float)
        ...
        # 逐渐过渡到默认状态的插值因子
        self.interp_factor = 0.0
        self.transition_speed = 0.005  # 每次更新增加的插值因子
        self.transitioning = True  # 标记是否正在过渡中
...
class InputThread(threading.Thread):
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            # 特殊命令的参数处理
            try:
                if cmd == "lp" and len(parts) >= 4 and self.left_controller:
                ...
                elif cmd == "rp" and len(parts) >= 4 and self.right_controller:
                    positions = list(map(float, parts[1:4]))
                    target_pos = np.array(positions)
                ...
                elif cmd == "lr" and len(parts) >= 2 and self.left_controller:
                    angle = float(parts[1])
                    self.left_controller.arm_controller.set_wrist_roll_angle(angle)

                elif cmd == "rr" and len(parts) >= 2 and self.right_controller:
                    angle = float(parts[1])
        ...
        try:
            while viewer.is_running():  # 只要查看器在运行就继续执行
                ...
                for i in range(model.nu):
                    if i >= 15 and i < 29:  # 左臂和左手关节 (15-28)
                        tau[i] = left_tau[i]
                    elif i >= 29 and i < 43:  # 右臂和右手关节 (29-42)
                        tau[i] = right_tau[i]
